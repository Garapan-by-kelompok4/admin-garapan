  ...
  const columns: ColumnDef<FlaggedContent>[] = [
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3 max-w-[260px]">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[11px] text-ink-400 mt-1 font-mono">ID: {row.original.id}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ getValue }: any) => {
        ...
        return (
          <span className={`inline-flex items-center justify-center h-5 px-2 rounded-full text-[11px] font-bold ${
            isHigh 
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4">
        ...
        <div className="flex bg-surface-3/50 p-1 rounded-lg border border-border/80 self-start select-none">
          {["Semua", "Ditinjau", "Aman", "Dihapus", "Disembunyikan"].map((st) => (
            <button
              key={st}
              onClick={() => {
                setStatusFilter(st);
        ...
        <DialogContent className="max-w-[850px] rounded-xl p-0 overflow-hidden border-border bg-white shadow-sh-3">
          {isLoadingDetail ? (
            <div className="p-12 text-center">
              ...
              <p className="text-xs text-ink-500 mt-2 font-medium">Memuat detail konten...</p>
            </div>
          ) : contentDetail ? (
            <div className="flex flex-col h-full max-h-[85vh]">
              ...
              <div className="flex-1 overflow-y-auto p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-4 pr-0 md:pr-2">
                  <h3 className="font-heading font-bold text-xs text-ink-400 uppercase tracking-wider">Preview Konten Jasa</h3>
                  <div className="rounded-lg border border-border p-4 bg-white space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-ink-900 text-sm leading-snug line-clamp-2">{contentDetail.title}</h4>
                    ...
                    <div className="space-y-1.5">
                      <span className="text-[11.5px] text-ink-400 font-semibold select-none">Deskripsi Jasa:</span>
                      <p className="text-xs text-ink-700 leading-relaxed font-medium bg-surface-2 p-3 rounded-lg border border-border/60 max-h-[140px] overflow-y-auto">
                        {contentDetail.description || "Tidak ada deskripsi jasa yang diberikan."}
                      </p>
                    </div>
                ...
                <div className="space-y-4">
                  <h3 className="font-heading font-bold text-xs text-ink-400 uppercase tracking-wider flex items-center gap-1.5">
                    <AlertTriangle className="h-4 w-4 text-warn-500" /> Rincian Laporan ({contentDetail.reportCount})
                  </h3>
                  <div className="space-y-3 max-h-[340px] overflow-y-auto pr-1">
                    {contentDetail.reports && contentDetail.reports.length > 0 ? (
                      contentDetail.reports.map((rep) => (
                        <div key={rep.id} className="p-3 border border-border rounded-lg bg-white space-y-2 hover:bg-surface-2 transition-all">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <div className={`h-6 w-6 rounded-full flex items-center justify-center text-white text-[9.5px] font-bold ${avatarClass(rep.reporterName)}`}>
                                {initials(rep.reporterName)}
                              </div>
                              <span className="text-xs font-semibold text-ink-900">{rep.reporterName}</span>
                            </div>
                            <span className="text-[10px] text-ink-400 font-medium">{formatDate(rep.createdAt)}</span>
                          </div>
              ...
              <div className="p-4 border-t border-border bg-surface-2/40 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      if (confirm("Apakah Anda yakin ingin menghapus jasa ini? Tindakan ini bersifat permanen.")) {
